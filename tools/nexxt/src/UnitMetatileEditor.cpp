...
void __fastcall TFormMetatileTool::Draw(void)
{
	...
	for(int j=0;j<d;j++) //tiles ín meta
	{
		...
		{
			//if(bTab2x2) tile= (metatileSet_2x2_id[tmpi+j+k*dimensions]);
			//if(bTab4x4) tile= (metatileSet_4x4_id[tmpi+j+k*dimensions]);
			//if(bTab8x8) tile= (metatileSet_8x8_id[tmpi+j+k*dimensions]);
			if(bTab2x2) tile= (metatileSet_2x2_id[tmpi]);
			if(bTab4x4) tile= (metatileSet_4x4_id[tmpi]);
			if(bTab8x8) tile= (metatileSet_8x8_id[tmpi]);
			...
			FormMain->DrawTile(Image1->Picture,xOff*m,yOff*m,tile,tile_highlight,pal,-1,-1,chrSelected[tile],false,m,false,true,false,false,false);
			tmpi++;
		}
	}
