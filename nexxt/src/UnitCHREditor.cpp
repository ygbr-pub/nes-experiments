...
void __fastcall TFormCHREditor::Draw(bool drawAll)
{
	...
	if(b){
		if(tg){
			...
			buffer->Canvas->LineTo(64+128+127+8, 64+127+8);
			buffer->Canvas->MoveTo(64+127+8, 64+8);
			buffer->Canvas->LineTo(64+127+8, 64+128+127+8);
		}
		if(mn){
			buffer->Canvas->Pen->Color = clGray;
			buffer->Canvas->Pen->Width = 1;
			...
			buffer->Canvas->MoveTo(64+128+127+8, 64+127+8);
			buffer->Canvas->LineTo(64+128+127+4, 64+127+8);

			buffer->Canvas->MoveTo(64+127+8, 64+128+127+8);
			buffer->Canvas->LineTo(64+127+8, 64+128+127+4);
    	}
	}
			...
			for(j=offx;j<lenx;j++)
			{
				if(buf[i][j]==254)
				{
					...
					if(bCway){
						...
						&& ((buf[i][j-1]==col || buf[i][j-1]==244) || (buf[i-1][j]==col || buf[i-1][j]==244)))
								buf[i-1][j-1]=254;
   ...
   FormMain->SetUndo();
	...
	for (y = 0; y < yTiles; y++)
	{
		for (x = 0; x < xTiles; x++)
		{
			...
			pp = tileViewTable[tileActive + x + y*16]*16 + bankViewTable[set + tileActive + x + y*16];
